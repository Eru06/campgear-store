...
const ALL_STATUSES: OrderStatus[] = ['pending_payment', 'confirmed', 'shipped', 'delivered', 'cancelled'];

export default function AdminOrderDetail() {
  const { id } = useParams<{ id: string }>();
  const { data: order, loading, error, refetch } = useApi<OrderResponse>(`/admin/orders/${id}`);
  ...
  return (
    <div>
      <Link to="/admin/orders" className="text-green-600 hover:underline text-sm">&larr; Back to orders</Link>
